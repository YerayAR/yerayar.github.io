(function () {
  ...
  function initContactForm() {
    ...
    const cooldownMs = 1000 * 60; // 1 min
    const lastSendKey = 'contact-last-send';
    ...
    contactForm.addEventListener('submit', async function (event) {
      ...
      const lastSend = parseInt(localStorage.getItem(lastSendKey) || '0', 10);
      if (Date.now() - lastSend < cooldownMs) {
        alert('Por favor espera un momento antes de volver a enviar.');
        return;
      }
      ...
      try {
        ...
        localStorage.setItem(lastSendKey, String(Date.now()));
